...
fis
  ...
  // 
  // .match("/components/**/(\w+).less", {
  .match(/\/components(\/\w+){1,}\/(\w+)\.less$/, {
  ...
  // .match("/components/**/(\w+).js", {
  .match(/\/components(\/\w+){1,}\/(\w+)\.js$/, {
    // useHash: true,
    // 压缩文件
  ...
  // .match(/\/components\/\w+\/img\/(.*)$/i, {
  // .match("/components/**/(\w+).{png,jpg,JPG,PNG,gif}", {
  .match(/\/components(\/\w+){1,}\/(\w+)\.{png,jpg,JPG,PNG,gif}$/, {
    release: '/${name}_${version}/images/c/$1'
  })
  .match('/components/(**).{html,tpl}', {
    release: false
    // release: '/${name}_${version}/views/c/$1'
  })
  // ************************************
  // *********业务逻辑*******************
  // ************************************
  ...
  .match(/^\/pages\/\w+\/(\w+)\.less$/i, {
    ...
    release: '/${name}_${version}/css/house/$1'
  })
  // js
  .match(/^\/pages\/\w+\/(\w+)\.js$/i, {
    ...
    // isViews : true
    release: '/${name}_${version}/js/house/$1'
  })
  // 图片
  .match(/^\/pages\/(\w+)\/img\/(.*)$/i, {
    // 启用hash值
    // useHash: true,
    release: '/${name}_${version}/images/house/$2'
  })
  // html
  .match(/^\/pages\/\w+\/(\w+)\.html$/i, {
    release: '/${name}_${version}/house/$1'
  })

  .match(/^\/dec\/\w+\/(\w+)\.less$/i, {
    parser: fis.plugin('less'),
    rExt: '.css',
    // useHash: true,
    useSprite: true,
    ...
    release: '/${name}_${version}/css/dec/$1'
  })
  .match(/^\/dec\/\w+\/(\w+)\.js$/i, {
    // 启用hash值
    // useHash: true,
    ...
    // isViews : true
    release: '/${name}_${version}/js/dec/$1'
  })
  .match(/^\/dec\/(\w+)\/img\/(.*)$/i, {
    // 启用hash值
    // useHash: true,
    release: '/${name}_${version}/images/dec/$2'
  })
  .match(/^\/dec\/\w+\/(\w+)\.html$/i, {
    release: '/${name}_${version}/dec/$1'
  })
  // ************通用***************
  // *******************************
  // 雪碧图
